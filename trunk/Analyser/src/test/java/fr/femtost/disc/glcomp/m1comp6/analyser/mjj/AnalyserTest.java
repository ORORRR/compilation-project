...
public class AnalyserTest {
    ...
    public void parseStringTest() throws AnalyserException {
        NodeMiniJaja root = analyser.parseString(
            "class C {\n" +
                "    final int x;\n" +
                "int t[3];\n" +
                "int f(boolean x) {\n" +
        ...
        // Construction of the expected AST
        ...
        // Check if the result of analysis is equivalent to the expected AST
        Assert.assertTrue(root.isEquivalentTo(nodeClass));

        // Check coordinates of some nodes in the AST
        Assert.assertEquals(1, root.getLine());
        Assert.assertEquals(0, root.getColumn());

        Assert.assertEquals(2, ((NodeDecls) ((NodeClass) root).getDecls()).getDecl().getLine());
        Assert.assertEquals(4, ((NodeDecls) ((NodeClass) root).getDecls()).getDecl().getColumn());
    }

    @Test(expected = AnalyserException.class)
    public void parseStringFailureTest() throws AnalyserException {
        analyser.parseString(
            "class C {" +
                "int x" +
                "main {" +
                "}" +
